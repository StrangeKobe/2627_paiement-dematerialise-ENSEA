...
from .forms import BilletForm, CategorieForm, EvenementForm, InfoPersonnelleForm, ProduitForm
from .forms import PoleForm
from .models import (
    Adhesion, Affectation, Categorie, EchecEncaissement, Evenement, JetonPaiement,
    LigneTransaction, ParticipantImporte, Pole, ProfilUtilisateur,
    Produit, Recharge, Transaction,
)
from .recus import envoyer_recu
from .roles import (
    annee_scolaire_courante, est_admin_ade, est_admin_ecole,
    peut_encaisser_adhesion_especes, peut_exporter, peut_gerer,
    peut_gerer_adhesions, peut_gerer_produits, peut_modifier_equipe,
    peut_modifier_parametres, peut_recharger_especes, peut_vendre,
    peut_voir_equipe, peut_voir_suivi_especes, poles_gerables, poles_vendables,
)
...
def asso_choix(request):
    ...
    on lui montre le recapitulatif des recettes et la liste des poles."""
...
def equipe_pole(request, slug):
    ...
    if request.method == "POST":
        ...
        if "ajouter" in request.POST:
            ...
            if affectation:
                # Un admin de pole a deja tous les droits "normaux" via
                # peut_gerer : seul "recharger en especes" a un effet reel
                # sur son compte, donc on ne touche aux six autres droits
                # que pour un vendeur.
        ...
        try:
            with db_transaction.atomic():
                ...
                for ligne in lignes_panier:
                    ...
                    if produit.evenement_id and not produit.evenement.est_vendable():
                        raise EchecEncaissement(
                            f"Les ventes pour {produit.evenement.nom} sont terminées."
                        )
                ...
                if not lignes_verifiees:
                    raise EchecEncaissement(
                        "Plus aucun produit disponible dans le panier, réessaie."
                    )
...
def gerer_produits(request, slug):
    ...
    # meme avec le droit de gerer le catalogue). Vue a part, sans filtre
    # par categorie : les produits retires n'apparaissent jamais dans
    # "Tous" ni dans les categories normales.
...
def modifier_produit(request, slug, produit_id):
    ...
    if request.method == "POST":
        if "retirer" in request.POST or "remettre" in request.POST:
            # Retirer (ou remettre) un article est reserve aux vrais
            # admins, jamais a un vendeur, meme avec le droit de gerer
            # le catalogue.
    ...
    if request.method == "POST" and request.FILES.get("fichier"):
        ...
        if col_nom is None:
            erreur = (
                ...
                "Colonnes détectées : " + ", ".join(entetes)
            )
        else:
            nb_importes = 0
...
def adherer_pole(request, slug):
            ...
            try:
                with db_transaction.atomic():
                    ...
                    if p.solde < tarif.prix:
                        raise EchecEncaissement(
                            f"Solde insuffisant pour cette adhesion de {tarif.prix} EUR."
                        )
                    p.solde -= tarif.prix
                    ...
                    try:
                        Adhesion.objects.create(
                            pole=pole, profil=p, annee=annee, montant=tarif.prix,
    ...
    return render(request, "caisse/adherer_pole.html", {
        "pole": pole, "annee": annee, "tarifs": tarifs,
        "deja_adherent": deja_adherent, "erreur": erreur,
    })
...
def gerer_adherents(request, slug):
    ...
    if request.method == "POST":
        if "ajouter_tarif" in request.POST:
            ...
            if utilisateur is None:
                ...
                try:
                    # When recording a cash payment, always attribute it to
                    # the current school year (annee_defaut), not necessarily
    ...
    # Years available for the selector: those with at least one adhesion
    # for this pole, plus the current school year.
...
def payer_adhesion_especes(request, slug):
    ...
    return render(request, "caisse/payer_adhesion_especes.html", {
        "pole": pole, "erreur": erreur, "tarifs": tarifs,
    })
