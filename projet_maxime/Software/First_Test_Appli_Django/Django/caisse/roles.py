...
def peut_gerer(user, pole):
    ...
    droits supplementaires a un vendeur (voir peut_modifier_equipe)."""
...
def peut_gerer_produits(user, pole):
    """Gerer le catalogue : produits, categories, evenements, billets."""
    if peut_gerer(user, pole):
        return True
...
def peut_gerer_adhesions(user, pole):
    """Prix d'adhesion, liste des adherents, export."""
    if peut_gerer(user, pole):
        return True
...
def peut_voir_equipe(user, pole):
    """Voir l'onglet Equipe (qui a quels droits sur ce pole). Ne donne
    jamais le droit de modifier quoi que ce soit : voir
    peut_modifier_equipe pour ca."""
    if peut_gerer(user, pole):
        return True
...
def peut_exporter(user, pole):
    """Exporter les ventes du pole au format Excel."""
    if peut_gerer(user, pole):
        return True
...
def peut_modifier_parametres(user, pole):
    """Modifier les parametres du pole (logo)."""
    if peut_gerer(user, pole):
        return True
