"""
Vues de l'espace admin école : le rôle technique qui gere les comptes, les
rôles et les codes de securite, sans jamais voir les prix ni la gestion
commerciale des pôles.
"""
    ...
    if request.method == "POST":
        if "ajouter" in request.POST:
            ...
            if utilisateur is None:
                ...
                erreur = (
                    "Un vendeur ou un admin de pôle doit obligatoirement être "
                    "rattaché a un pôle précis. Choisis un pôle dans la liste."
                )
            else:
...
def ecole_profils(request):
    """Corriger le nom, prenom, email ou date de naissance de n'importe
    quel compte, meme deja ancre. Reserve a l'admin école."""
    _verifier_acces(request)
...
def ecole_comptes(request):
    ...
    return render(request, "caisse/ecole_comptes.html", {
        "profils": profils, "nb_a_supprimer": nb_a_supprimer, "nb_bientot": nb_bientot,
        "recherche": recherche,
    })
